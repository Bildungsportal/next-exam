...
/** Guest vCPUs: host cores minus 2 (headroom for host+QEMU), clamped 4..8 (never below previous fixed 4). */
export function getQemuVcpuCount() {
    if (cachedVcpuCount !== null) return cachedVcpuCount;
    let hostCores = 0;
    try {
        hostCores = typeof os.availableParallelism === 'function' ? os.availableParallelism() : os.cpus().length;
    } catch (e) {
        hostCores = 0;
    }
    if (!Number.isFinite(hostCores) || hostCores < 1) hostCores = 0;
    const vcpus = hostCores ? Math.min(8, Math.max(4, hostCores - 2)) : 4;
    cachedVcpuCount = vcpus;
    return vcpus;
}

export function getQemuSmpArgs() {
    const vcpus = getQemuVcpuCount();
    if (process.platform === 'win32') return ['-smp', `cores=${vcpus},threads=1`];
    return ['-smp', String(vcpus)];
}
