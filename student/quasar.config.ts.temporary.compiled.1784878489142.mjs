...
var package_default = {
  ...
  scripts: {
    ...
    "build:linux:iosbranch": "npm run prebuild && npm run clean && quasar build --verbose -m electron -l && npm run protect:main",
    "build:mac:x64:iosbranch": "npm run prebuild && npm run clean && quasar build --verbose -m electron -m --arch x64 && npm run protect:main",
    "build:mac:arm64:iosbranch": "npm run prebuild && npm run clean && quasar build --verbose -m electron -m --arch arm64 && npm run protect:main",
    "build:win:portable:iosbranch": "npm run prebuild && npm run cleanwin && quasar build --verbose -m electron -w portable:x64 && npm run protect:main",
    "build:win:msi:iosbranch": "npm run prebuild && npm run cleanwin && quasar build --verbose -m electron -w msi:x64 && npm run protect:main",
    "build:ios": "npm run prebuild && quasar build -m capacitor -T ios --skip-pkg",
...
import fs from "fs";
var __quasar_inject_import_meta_url__ = "file:///C:/Users/valueerror/Documents/GIT/next-exam/student/quasar.config.ts";
  ...
  return {
    ...
    build: {
      ...
      extendViteConf(viteConf) {
        ...
        if (ctx.mode.electron && ctx.prod) {
          ...
          viteConf.plugins.push({
            ...
            generateBundle(_, bundle) {
              const rewrite = (s) => s.replace(/`\/img\//g, "`./img/").replace(/\\"\/img\//g, '\\"./img/').replace(/\\'\/img\//g, "\\'./img/").replace(/"\/img\//g, '"./img/').replace(/'\/img\//g, "'./img/");
              for (const item of Object.values(bundle)) {
                const entry = item;
      ...
      rollupOptions: {
        ...
        external: ctx.mode.electron ? ["electron", ...builtinModules, ...Object.keys(package_default.dependencies || {})] : []
      }
    },
    ...
    electron: {
      ...
      builder: {
        ...
        extraResources: [
          ...
          ...["assessment-helper.app"].filter((name) => fse.existsSync(path.join(__dirname, "scripts/apple", name))).map((name) => ({ from: `scripts/apple/${name}`, to: `apple/${name}` }))
        ],
