...
export default defineConfig(( ctx: any ) => {
  return {
    ...
    build: {
      ...
      extendViteConf (viteConf: any) {
        ...
        viteConf.resolve.alias = {
            ...
            pkijs: path.resolve(__dirname, 'node_modules/pkijs'),
            asn1js: path.resolve(__dirname, 'node_modules/asn1js'),
            '@signpdf/placeholder-pdf-lib': path.resolve(__dirname, 'node_modules/@signpdf/placeholder-pdf-lib'),
            '@signpdf/signpdf': path.resolve(__dirname, 'node_modules/@signpdf/signpdf'),
            '@signpdf/utils': path.resolve(__dirname, 'node_modules/@signpdf/utils'),
        ...
        // Capacitor/iOS: exclude desktop-only runtime assets from the web bundle.
        if (ctx.mode.capacitor) {
          viteConf.build.copyPublicDir = false;
          viteConf.plugins = viteConf.plugins || [];
          viteConf.plugins.push({
            name: 'capacitor-public-assets',
            writeBundle() {
              const publicDir = path.resolve(__dirname, 'public');
              const capacitorWww = path.resolve(__dirname, 'src-capacitor', 'www');
              for (const name of ['GeoGebra', 'img', 'icons', 'attention.wav', 'demo.pdf', 'Testfile.docx', 'Testfile.odt']) {
                fse.copySync(path.join(publicDir, name), path.join(capacitorWww, name));
              }
            }
          });
          viteConf.plugins.push({
        ...
        if (ctx.mode.electron && ctx.prod) {
          ...
          // string-array obfuscation of the one renderer chunk carrying the app secret, so it is not grep-able in the bundle
          viteConf.plugins = viteConf.plugins || [];
          viteConf.plugins.push({
            name: 'obfuscate-secret-chunk',
            enforce: 'post',
            async generateBundle(_: unknown, bundle: Record<string, any>) {
              const JsObfuscator = (await import('javascript-obfuscator')).default;
              for (const item of Object.values(bundle)) {
                if (item?.type !== 'chunk' || !item.code || !item.code.includes('x-next-exam-app-mac')) continue;
                item.code = JsObfuscator.obfuscate(item.code, {
                  target: 'browser',
                  compact: true,
                  stringArray: true,
          ...
          viteConf.base = './';
          // base './' drops the dynamic-import dep map, so lazy route CSS (ExamHeader in WebviewPane chunk) was never linked -> exam header lost its flex layout. One index.css in index.html instead.
    ...
    capacitor: {
      ...
      ...(process.env.NXE_FAST_IOS === '1' ? { capacitorCliPreparationParams: ['copy', 'ios'] } : {})
    },
    ...
    electron: {
      ...
      inspectPort: Number(process.env.NXE_ELECTRON_INSPECT_PORT || 5858),
      ...
      builder: {
        ...
        win: {
          ...
          // SIGN env (SIGN !== 'false'): signAndEditExecutable gates signing; Azure Trusted Signing auth comes from AZURE_* env vars (OIDC in CI)
          signAndEditExecutable: signEnabled,
          // electron-builder only warns on signing failures; fail the build instead of shipping unsigned artifacts
          forceCodeSigning: signEnabled,
          ...(signEnabled && {
            azureSignOptions: {
              publisherName: 'Open Source Open Schools (OSOS) Austria',
              endpoint: 'https://neu.codesigning.azure.net/',
              codeSigningAccountName: 'Weissel-Thomas',
              certificateProfileName: 'next-exam',
            },
          }),
        },
