...
function checkBasicAuth(req, res, basicUser, basicPass) {
    ...
    // hex password: compare case-insensitively, the Windows guest reads the SMBIOS serial back uppercased
    if (u !== basicUser || p.toLowerCase() !== basicPass.toLowerCase()) {
        res.setHeader('WWW-Authenticate', 'Basic realm="exam"');
        res.writeHead(401);
        res.end();
        return false;
 ...
 * @param {{ rootDir: string, port?: number, mountPath?: string, basicUser: string, basicPass: string }} opts
 */
export function startExamWebdav(opts) {
    ...
    const basicUser = opts.basicUser;
    const basicPass = opts.basicPass;

    if (!basicUser || !basicPass) {
        throw new Error('examWebdavServer: session credentials required');
    }
    ...
    server.listen(port, '127.0.0.1', () => {
        log.info(`examWebdavServer: listening 127.0.0.1:${port} mount=${mountPath} root=${rootDir}`);
    });
    server.on('error', (e) => {
