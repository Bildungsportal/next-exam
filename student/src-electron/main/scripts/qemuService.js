...
function spawnLogged(cmd, args, options = {}) {
    const loggedArgs = args.map((arg) => String(arg).replace(/serial=NXEWD-[0-9a-f]+/i, 'serial=NXEWD-[redacted]'));
    log.info(`qemuService: spawn ${cmd} ${loggedArgs.join(' ')}`);
...
async function startHeadless({
    ...
    const webdavUser = 'nxe';
    // 16 bytes = 32 hex chars: the Windows guest truncates the SMBIOS type-1 serial at 64 chars,
    // and the serial carries the 'NXEWD-' prefix, so a longer password would arrive cut off.
    const webdavPassword = crypto.randomBytes(16).toString('hex');
    startExamWebdav({
        rootDir: examdirectory,
        port: EXAM_WEBDAV_PORT,
        mountPath: EXAM_WEBDAV_MOUNT_PATH,
        basicUser: webdavUser,
        basicPass: webdavPassword,
    });
    ...
    const args = [
        ...
        '-smbios', `type=1,serial=NXEWD-${webdavPassword}`,
        ...netArgs,
        ...getQemuUsbTabletArgs(),
        ...getQemuLegacyBootOrderArgs(),
    ];
