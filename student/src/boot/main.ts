...
export default defineBoot(async ( { app, router } ) => {
    const options = {
        ...
        didOpen: (popup) => {
            if (isIOS()) window.ipcRenderer.invoke('set-native-webviews-hidden-for-popup', true);
        ...
        didClose: () => {
            if (isIOS()) window.ipcRenderer.invoke('set-native-webviews-hidden-for-popup', false);
        }
    };
