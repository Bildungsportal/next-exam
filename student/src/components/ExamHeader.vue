    ...
    <div id="apphead" class="bg-dark">
        ...
        <div v-if="serverstatus?.useExamSections" class="header-sections me-2">
            <template v-for="n in 4" :key="n">
                <div v-if="serverstatus?.examSections?.[n] && !serverstatus.examSections[n].sectiondisabled"
  ...
  export default {
    ...
    methods: {
      ...
      async switchExamSection(sectionNumber) {
        ...
        if (isIOS()) await window.ipcRenderer.invoke('set-native-webviews-hidden-for-popup', true);
        await this.$nextTick();
        this.$swal.fire({
          ...
          if (isIOS()) {
            await this.$nextTick();
            await window.ipcRenderer.invoke('set-native-webviews-hidden-for-popup', false);
          }
