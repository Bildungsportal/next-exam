<template>
    <div class="embed-container pdfview-pane-rendered" @click.stop>
        ...
        <li v-if="!localLockdown && toolbar.showSend" class="nav-item">
            <div class="nav-link btn btn-success btn-sm unstyled unstyled-send" id="send-button" aria-label="e2e-send-exam" @click="printBase64()" :title="$t('editor.send')">
        ...
        >
        <div
            :class="['pdf-page-wrapper', { 'annotation-drawing-active': isDrawingToolActive }]"
            :style="{ width: page.width + 'px', height: page.height + 'px', transform: `scale(${zoom})` }"
            @pointerdown="tool !== 'delete' && tool !== 'text' ? startDraw($event, pageIndex) : null"
            @pointermove="isDrawing ? updateDraw($event, pageIndex) : null"
            @pointerup="isDrawing ? finishDraw($event, pageIndex) : null"
            @pointercancel="isDrawing ? cancelDraw() : null"
