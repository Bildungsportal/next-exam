    ...
    <div id="pdfrenderer" class="pdf-overlay-root">
        ...
        <div v-else-if="parsedPages.length > 0" class="pdf-scroll-container">
            <div
                ...
                :key="pageIndex"
                :class="['pdf-page-wrapper', { 'annotation-drawing-active': enableAnnotations && isDrawingToolActive }]"
                :style="{ width: page.width + 'px', height: page.height + 'px' }"
                @pointerdown="enableAnnotations && tool !== 'delete' && tool !== 'text' ? startDraw($event, pageIndex) : null"
                @pointermove="enableAnnotations && isDrawing ? updateDraw($event, pageIndex) : null"
                @pointerup="enableAnnotations && isDrawing ? finishDraw($event, pageIndex) : null"
                @pointercancel="enableAnnotations && isDrawing ? cancelDraw() : null"
            ...
            >
                ...
                <div
                    v-for="entry in orderedFieldsForPage(page, pageIndex)"
                    :key="entry.field.id"
                    v-show="!isBlacklisted(entry.field.id)"
                    :class="['input-overlay', isCheckboxOverlay(entry) ? 'checkbox-overlay' : '']"
...
export default {
    ...
    methods: {
        ...
        orderedFieldsForPage(page, pageIndex) {
            return mergePageOverlayFields(page, this.customFields, pageIndex);
        },
...
.pdf-page-wrapper.annotation-drawing-active {
    touch-action: none;
}
