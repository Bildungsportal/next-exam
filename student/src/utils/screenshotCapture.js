...
let allowWindowFallbackOnStreamFailure = false;
...
/** Linux Wayland: if getDisplayMedia/portal fails, allow capturePage window fallback (same IPC as Cage). */
export function setAllowWindowFallbackOnStreamFailure(enabled) {
  allowWindowFallbackOnStreamFailure = !!enabled;
}
...
export async function initDisplayStreamOnce() {
  ...
  if (acquired) {
    ...
    initAttempted = false;
    if (linuxKioskRunningInCage || allowWindowFallbackOnStreamFailure) {
      setCageWindowCaptureFallback(true);
      loggingBridge.info('screenshotCapture @ initDisplayStreamOnce: using capturePage fallback');
    } else {
...
export async function ensureDisplayStreamAsync() {
  // macOS / Cage / Wayland portal-failure: capturePage path needs no getDisplayMedia stream.
