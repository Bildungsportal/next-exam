...
import iosUpdateListener from "./ios/iosUpdateListener.ts";
...
export async function loadPDF(file, base64 = false, zoom=180, submission=false, type="send"){
    ...
    if (base64){
        ...
        let data = isIOS() ? await iosUpdateListener.getPdfAsync(file) : await window.ipcRenderer.invoke('getpdfasync', file )
        let isvalid = data && isValidPdf(data)
        if (!isvalid){
            this.$swal.fire({
...
function processNode(node) {           
    ...
    if (nodestring.includes("data:image")){
        ...
        loggingBridge.info("found table")
...
export async function loadHTML(file){
    let data = isIOS() ? await iosUpdateListener.getFilesAsync(file) : await window.ipcRenderer.invoke('getfilesasync', file )
    console.log("filehandler @ loadHTML: ", data);
    this.LTdisable()
    this.$swal.fire({
                    ...
                    try {
                        ...
                        if (base64Data) {
                            ...
                            audioPlayer.play().then(() => { 
                                loggingBridge.info('filehandler @ playAudio: Playback started');
...
export async function loadGGB(file, base64=false){
    ...
    .then(async (result) => {
        if (result.isConfirmed) {
            ...
            if (!base64){
                const loadResult = isIOS() ? await iosUpdateListener.loadGGB(file) : await window.ipcRenderer.invoke('loadGGB', file);
                loggingBridge.info("filehandler @ loadGGB: check loadGGB result: ", loadResult);
...
export async function getExamMaterials() {
    ...
    loggingBridge.info('filehandler @ getExamMaterials: got materials:', examMaterials?.materials?.map(m => m.filename))
    if (examMaterials) {
        ...
        //     loggingBridge.info("filehandler @ getExamMaterials: allowedUrls are identical - skipping webview blocking setup");
        //     return;
        // }


        loggingBridge.info("filehandler @ getExamMaterials: received new examMaterials")
        ...
        if (webviewPane) {
            loggingBridge.info('filehandler @ getExamMaterials: setting WebviewPane dom-ready event to block websites');
            ...
            // loggingBridge.info('filehandler @ getExamMaterials: 1')
            ...
            webviewPane._blockingDomReadyHandler = async () => {  // content id can only be accessed after dom-ready event                
                ...
                //loggingBridge.info('filehandler @ getExamMaterials: 2')
                    ...
                    for (let i = 0; i < retries; i++) {
                        if (webviewPane.getWebContentsId) {
                            ...
                            if (guestId) {
                                ...
                                // loggingBridge.info('filehandler @ getExamMaterials: 3')
                                ...
                                loggingBridge.info(`filehandler @ getExamMaterials: started blocking for WebviewPane ${guestId}`);
                                return;
                            }
                        }
                        // wait before retry
            ...
            // loggingBridge.info('filehandler @ getExamMaterials: 4')
            webviewPane.addEventListener('dom-ready', webviewPane._blockingDomReadyHandler);

        } else {
            loggingBridge.info('filehandler @ getExamMaterials: WebviewPane not in DOM');
        }
