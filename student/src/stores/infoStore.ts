...
export const useInfoStore = defineStore("info", {
    state: () => ({
        ...
        numberOfFiles: 0 as number,
        submissionnumber: 0 as number,
        bipUserId: "" as string,
    }),
    actions: {
        ...
        async updateInfo(): Promise<boolean> {
            ...
            if (response) {
                ...
                // Keep the last known server/pin after a disconnect reset so the reconnect dialog can prefill them.
                this.serverip = clientinfo.serverip || this.serverip;
                this.servername = clientinfo.servername;
                this.servertoken = clientinfo.servertoken;
                this.clientname = clientinfo.name;
                this.pincode = clientinfo.pin || this.pincode;
                ...
                this.submissionnumber = clientinfo?.submissionnumber;
                this.encryptionPassword = response.serverstatus.encryptionPassword;
                this.password = response.serverstatus?.password;

                // this is only filled in ios response, there level is left as is for other cases
