...
<div class="geogebra-page-root">
    ...
    <div id="toolbar" class="d-inline p-1">
        ...
        <div class="btn-group  ms-2 me-1 mb-0 " role="group">
            <div class="btn btn-outline-info btn-sm p-0 pe-2 ps-1  mb-0" @click="setsource('suite')"> <img src="/img/svg/formula.svg" class="" width="20" height="20" >suite</div>
            <div class="btn btn-outline-info btn-sm p-0 pe-2 ps-1  mb-0" @click="setsource('classic')"> <img src="/img/svg/formula.svg" class="" width="20" height="20" >classic</div>
        </div>
        ...
        <div v-for="file in localfiles" class="d-inline mb-0">
            <div v-if="(file.type === 'pdf')"   :class="{'bg-warning': file.name == currentFile}" class="btn btn-info p-0 pe-2 ps-1 ms-1 mb-0 btn-sm" @click="selectedFile=file.name; loadPDF(file.name)"><img src="/img/svg/document-replace.svg" class="" width="20" height="20" > {{file.name}} </div>
            <div v-if="(file.type === 'ggb')"   :class="{'bg-warning': file.name == currentFile}" class="btn btn-info p-0 pe-2 ps-1 ms-1 mb-0 btn-sm" @click="selectedFile=file.name; loadGGB(file.name)"><img src="/img/svg/document-replace.svg" class="" width="20" height="20" > {{file.name}} </div>
    ...
    <Transition name="clipboard-slide">
      ...
      >
        ...
        <div class="customClipboard__list">
          ...
          >
            <img
              src="/img/svg/edit-paste-style.svg"
              alt=""
              width="16"
              height="16"
...
export default {
    ...
    setup() {
      ...
      let {online, examtype, servername, serverip, token, clientname, serverstatus, pincode, localLockdown, battery, wlanInfo, entrytime} = storeToRefs(infoStore);
    ...
    methods: {
        ...
        async loadFilelist(){
            let filelist = isIOS() ? await iosUpdateListener.getFilesAsync(null) : await window.ipcRenderer.invoke('getfilesasync', null)
        ...
        async initGeoGebra(appName = 'suite') {
            ...
            const enable3D = section?.[groupKey]?.examConfig?.math?.enable3D !== false
            const params = {
                "appName": appName,
                "language": "de",
        ...
        async saveGGB(filename, base64GgbFile, saveReason) {
            if (isIOS()) {
                return iosUpdateListener.saveGGB(filename, base64GgbFile, saveReason)
            }
            return await window.ipcRenderer.invoke('saveGGB', {
                filename,
        ...
        async saveSectionSwitchBackup() {
            ...
            const response = await this.saveGGB(filename, base64GgbFile, 'sectionswitch');
            ...
            if (reason == "manual" ){ 
                await this.$swal({
                    ...
                    if (result.isConfirmed) {
                        ...
                        let response = await this.saveGGB(filename, base64GgbFile, 'manual');
                ...
                if (!base64GgbFile) {
                    ...
                    let response = await this.saveGGB(filename, base64GgbFile, saveReason);
                    if (response && response.status === "success" && reason == "manual" ){  // we wait for a response - only show feed back if manually saved
                        this.loadFilelist();
                        this.$swal.fire({
                            title: this.$t("editor.saved"),
