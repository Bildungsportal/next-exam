...
export default {
    ...
    methods: {
        ...
        async sendExamToTeacher(directsend = false, type = "send") {
            ...
            if (type === 'print') {
                const response = await this.getBase64PDF(pdfArgs, type)
    ...
    async mounted() {
        ...
        if (!this.development) {
            this._onFocusLostMouseleave = (e) => this.sendFocuslost(false, { source: `mouseleave x=${e?.clientX} y=${e?.clientY} rel=${e?.relatedTarget?.nodeName || 'none'}` });
