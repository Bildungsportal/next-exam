    ...
    <div class="w-100 p-0 m-0 text-white shadow-sm text-center"
        ...
        <div v-if="editor" class="m-2" id="editortoolbar" style="text-align:left;">
            ...
            <div>
                ...
                <div v-if="!localLockdown" id="sendfinalexam" aria-label="e2e-finish-exam"
                     class="invisible-button btn btn-outline-success p-0 ms-1 me-1 mb-0 btn-sm pe-2 ps-1 "
    ...
    <div v-if="!splitview" id="editormaincontainer"
        ...
        <div id="editorcontainer" class="shadow" style="">
            ...
            <div v-if="editor" :key="'main-' + (editor ? 'ready' : '')">
                <editor-content :editor="editor" class='p-0' id="editorcontent" aria-label="e2e-editor"
...
export default {
    ...
    setup() {
      let {development, serverApiPort, electron, hostip} = storeToRefs(useConfigStore());

      const infoStore = useInfoStore();
      infoStore.online = true;
      infoStore.componentName = "Writer";
      let {online, examtype, servername, serverip, token, clientname, serverstatus, pincode, localLockdown, battery, wlanInfo, entrytime, componentName} = storeToRefs(infoStore);
    ...
    methods: {
        ...
        stopSplitResize() {
            ...
            // pane width changed -> text reflowed, highlight positions are stale
            this.$nextTick(() => this.LTupdateHighlights());
        },
        ...
        setCursorAtStartOfRange(range) {
            ...
            // scrollIntoView also moves .editor-root (overflow:hidden) by header+toolbar
            const scroller = document.getElementById('editormaincontainer');
            if (scroller?.parentElement) scroller.parentElement.scrollTop = 0;
            if (!scroller) return;
            const rect = range.getBoundingClientRect();
            const crect = scroller.getBoundingClientRect();
            scroller.scrollTo({
                top: scroller.scrollTop + rect.top + rect.height / 2 - (crect.top + crect.height / 2),
                behavior: 'smooth',
            });
        },
        ...
        async loadFilelist() {
            this.localfiles = isIOS() ? await iosUpdateListener.getFilesAsync(null) : await window.ipcRenderer.invoke('getfilesasync', null);
        ...
        async saveContent(backup, why) {
            ...
            if (isIOS()) {
                const returnValue = await iosUpdateListener.storeHTML(filename, editorcontent, why);
                loggingBridge.info("editor @ ios storeHTML: ", returnValue)
                switch (returnValue.returnSignal) {
                    case "loadfilelist":
                        this.loadFilelist();
                        break;
                    case "fileerror":
                        this.handleFileerror(returnValue.args.message);
                        break;
                    case "error":
                        loggingBridge.error("editor @ ios storeHTML generic error: ", returnValue.args.error);
                        break;
                    default:
                        loggingBridge.error("editor @ ios storeHTML returnSignal not implemented: ", returnValue);
                        break;
                }
            }
        ...
        async getBase64PDF(pdfArgs, reason) {
            if (isIOS()) {
                return await iosUpdateListener.getBase64PDF(pdfArgs, reason);
            }
            return await window.ipcRenderer.invoke('getPDFbase64', {...pdfArgs, reason: reason});
        },
        ...
        async sendExamToTeacher(directsend = false, type = "send") {
            ...
            if (type === 'print') {
                ...
                let response
                try {
                    response = await this.getBase64PDF(pdfArgs, type)
                } finally {
                    this.$swal.close()
                }
            ...
            let response
            try {
                response = await this.getBase64PDF(pdfArgs, 'previewSigned');
            } finally {
                this.$swal.close()
            }
            if (response?.status !== 'success') {
        ...
        async toggleSplitview() {
            this.LTdisable();  //close lt - must run before the v-if switch, otherwise it cleans up the canvas that is about to be destroyed
        ...
        async sendFocuslost(ctrlalt = false, options = {}) {
            ...
            loggingBridge.warn(
                `editor @ sendFocuslost: source=${source} ctrlalt=${ctrlalt} hidden=${document.hidden} visibility=${document.visibilityState} swal=${document.body.classList.contains('swal2-shown')} body=${Math.round(b.width)}x${Math.round(b.height)} win=${window.innerWidth}x${window.innerHeight} hasFocus=${document.hasFocus()}`
            );
        ...
        handleVisibilityChange() {
            // log every transition (not just hidden) - occlusion by a foreign window hides the page without stealing focus
            loggingBridge.warn(`editor @ visibilitychange: state=${document.visibilityState} hasFocus=${document.hasFocus()}`);
            if (document.hidden) {
        ...
        async loadBackupFile(filename = false) {
            ...
            try {
                let backupPromise = isIOS() ? iosUpdateListener.getBackupFile(backupfileName) : window.ipcRenderer.invoke('getbackupfile', backupfileName);
                const [backupfileContent, ready] = await Promise.all([
                    backupPromise,
                    this.waitForEditorReady(),
                ]);

                console.log(`editor @ loadBackupFile - backupFileContent: `, backupfileContent);
                if (!ready) return;
        ...
        handleFileerror(msg) {
            loggingBridge.error('editor @ fileerror: ', msg.message);
    ...
    async mounted() {
        ...
        if (!this.development) {
            // DOM-Umbau (pdf preview) feuert synthetisches mouseleave mit Cursor mitten im Fenster - nur Randaustritt ist echt
            this._onFocusLostMouseleave = (e) => {
                const edge = 2;
                const inside = e?.clientX > edge && e?.clientY > edge
                    && e?.clientX < window.innerWidth - edge && e?.clientY < window.innerHeight - edge;
                if (inside) {
                    loggingBridge.warn(`editor @ mouseleave ignored: synthetic (x=${e?.clientX} y=${e?.clientY} win=${window.innerWidth}x${window.innerHeight})`);
                    return;
                }
                this.sendFocuslost(false, { source: `mouseleave x=${e?.clientX} y=${e?.clientY} rel=${e?.relatedTarget?.nodeName || 'none'}` });
            };
