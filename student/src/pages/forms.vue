    ...
    <div class="column" style="height: 100%">
        ...
        <div id="toolbar" class="d-inline p-1">
            <button class="btn btn-primary p-0 pe-2 ps-1 me-1 mb-0 btn-sm" @click="reloadWebview"
                    :title="$t('website.reloadwebview')"><img src="/img/svg/edit-redo.svg" class="" width="22"
            ...
            <div v-for="file in examMaterials" :key="file.filename" class="d-inline" style="text-align:left">
                ...
                <div v-if="(file.filetype == 'pdf')" class="btn btn-outline-cyan p-0 pe-2 ps-1 me-1 mb-0 btn-sm"
                     @click="selectedFile=file.filename; loadBase64file(file)"><img src="/img/svg/eye-fill.svg"
                                                                                    class="grey" width="22" height="22"
                                                                                    style="vertical-align: top;">
                    {{ file.filename }}
                </div>
                <div v-if="(file.filetype == 'audio')" class="btn btn-outline-cyan p-0 pe-2 ps-1 me-1 mb-0 btn-sm"
                     @click="loadBase64file(file)"><img src="/img/svg/im-google-talk.svg" class="" width="22"
            ...
            <div v-for="file in localfiles" class="d-inline mb-0">
                <div v-if="(file.type == 'pdf')" class="btn btn-info p-0 pe-2 ps-1 ms-1 mb-0 btn-sm"
                     @click="selectedFile=file.name; loadPDF(file.name)"><img src="/img/svg/document-replace.svg"
                                                                              class="" width="20" height="20">
                    {{ file.name }}
                </div>
                <div v-if="(file.type == 'image')" class="btn btn-info p-0 pe-2 ps-1 ms-1 mb-0 btn-sm"
                     @click="loadImage(file.name)"><img src="/img/svg/eye-fill.svg" class="white" width="22"
        ...
        <div id="content">
            ...
            <!-- focuswarning end  -->
            <webview v-if="!isIOSPlatform" ref="wvmain" id="formswebview" autosize="on" allowpopups
            ...
            <div v-else ref="nativeSlot" id="formswebview"></div>
...
const iosFormsScript = `
    (() => {
        const formElement = document.querySelector('form');
        const nextElement = formElement ? formElement.nextElementSibling : null;
        if (nextElement) nextElement.style.display = 'none';
        const style = document.createElement('style');
        style.textContent = \`
            [role="link"], #branding-footer, #FormTitleId_EnableScreenReader,
            button[aria-label="Formularmenü"], button[aria-label="Form menu"],
            ...
            div[data-report-abuse-url] button[aria-label="Help & feedback"] { display: none !important; }
        \`;
...
export default {
    ...
    setup() {
        let {development, serverApiPort, electron, hostip, showdevtools} = storeToRefs(useConfigStore());

        const infoStore = useInfoStore();
        infoStore.online = true;
        infoStore.componentName = "Forms";
        let {examtype, servername, servertoken, serverip, token, clientname, serverstatus, clientApiPort, pincode, localLockdown, online, battery, wlanInfo, entrytime} = storeToRefs(infoStore);

        return { development, serverApiPort, electron, hostip, showdevtools, isIOSPlatform: isIOS(),
    ...
    async mounted() {
        ...
        this.$nextTick(async () => { // Code that will run only after the entire view has been rendered
            ...
            window.ipcRenderer.on('getmaterials', async () => {
                await this.getExamMaterials()
                if (isIOS()) await this.resizeNativeBrowser()
            });
            ...
            if (isIOS()) {
                this.autoEventListener(window, 'resize', this.resizeNativeBrowser)
                await this.openNativeBrowser()
            }
    ...
    methods: {
        ...
        async loadFilelist() {
            this.localfiles = isIOS()
                ? await iosUpdateListener.getFilesAsync(null)
                : await window.ipcRenderer.invoke('getfilesasync', null)
        },
        ...
        loadFormsWebview() {
            if (!this.formsUrl) return;
            if (isIOS()) {
                WebViewBox.reload({url: this.formsUrl})
                return;
            }
            if (!this.$refs.wvmain) return;
        ...
        async fetchInfo() {
            ...
            if (this.formsUrl && (this.$refs.wvmain || isIOS())) {
        ...
        // Opens the iOS Forms WKWebView in the renderer placeholder.
        async openNativeBrowser() {
            if (!this.formsUrl || !this.$refs.nativeSlot) return;
            const rect = this.$refs.nativeSlot.getBoundingClientRect();
            await WebViewBox.open({
                ...
                executeJavaScript: iosFormsScript,
                mode: 'forms',
                showdevtools: this.showdevtools,
            });
        },

        // Keeps the native Forms view aligned after rotation or resize.
        async resizeNativeBrowser() {
            if (!isIOS() || !this.$refs.nativeSlot) return;
            const rect = this.$refs.nativeSlot.getBoundingClientRect();
            await WebViewBox.resize({x: rect.left, y: rect.top, width: rect.width, height: rect.height});
        },

        // Hides native Forms while a material preview is open.
        async syncNativeBrowserVisibility() {
            if (!isIOS()) return;
            if (this.pdfPreviewState != null || this.webviewVisible) await WebViewBox.hide();
            else await WebViewBox.show();
        },
