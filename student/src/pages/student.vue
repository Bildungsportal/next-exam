    ...
    <div v-show="!isLoading" id="wrapper" class="w-100 h-100 d-flex">
        ...
        <div class="p-3 text-white bg-dark h-100 student-sidebar" style="width: 240px; min-width: 240px;">
            ...
            <div :class="(token)? 'disabledexam':''" class="form-check form-switch m-1 mb-2 mt-2">
                <input id="manualsearch" aria-label="e2e-manual-search" type="checkbox" v-model="advanced" class="form-check-input"
            ...
            <div class="col-8 mb-2" :class="(token)? 'disabledtext':''">
                <div v-if="!bipToken" class="input-group  mb-1">
                    ...
                    <input ref="userInput" :value="username" @input="onUsernameInput" @focus="onUsernameFocus" @paste.prevent @drop.prevent type="text"
                           required="required" maxlength="25" class="form-control" id="user" aria-label="e2e-user" placeholder=""
                ...
                <div class="input-group  mb-1">
                    ...
                    <input v-model="pincode" type="text" inputmode="numeric" pattern="[0-9]*" maxlength="4"
                           class="form-control" id="pin" aria-label="e2e-pin" placeholder=""
                ...
                <div v-if="advanced" class="input-group  mb-1">
                    ...
                    <input :class="{'form-control': validip, 'form-control is-invalid': !validip}" v-model="serverip"
                           class="form-control" id="serverip" aria-label="e2e-server-ip" placeholder=""
                           style="width:135px;max-width:135px;min-width:135px;">
                </div>
            </div>
                    ...
                    <div v-for="server in serverlist" :key="server.id || server.servername"
                        ...
                        <div
                            ...
                            <img v-if="!server.reachable" src="/img/svg/emblem-warning.svg"
                                 :title="server.certChanged ? $t('student.certchanged') : $t('student.unreachable')"
                            ...
                            <div v-if="!token" style="margin-top:2px; padding:0px; ">
                                <!-- not logged in, no bip server --> <input v-if="!server.bip" style="width:200px;"
                                                                             :id="server.servername" :aria-label="'e2e-register-' + server.servername" type="button"
                                                                             name="register" class="btn btn-sm btn-info"
                                                                             :value="$t('student.register')"
                                                                             @click="registerClient(server.serverip,server.servername)">
                                <!-- not logged in, bip server, portal status is not open (closed wins over restricted) --> <input v-if="server.bip && server.examStatus && server.examStatus !== 'open'" style="width:200px;"
                                                                                       :id="server.servername" type="button"
                                                                                       name="register" class="btn btn-sm btn-secondary"
                                                                                       :value="server.examStatus"/>
                                <!-- not logged in, bip server, open but student not allowed (no bip login / exam not assigned) --> <input v-if="server.bip && (!server.examStatus || server.examStatus === 'open') && isBipExamRestricted(server)" style="width:200px;"
                                                                                       :id="server.servername" type="button"
                                                                                       name="register" class="btn btn-sm btn-secondary"
                                                                                       value="restricted"/>
                                <!-- not logged in, bip server, allowed but no LAN teacher --> <input v-if="server.bip && (!server.examStatus || server.examStatus === 'open') && !isBipExamRestricted(server) && !isBipExamLanJoinable(server)" style="width:200px;"
                                                                                                                      :id="server.servername" type="button"
                                                                                                                      name="register" class="btn btn-sm btn-secondary"
                                                                                                                      value="offline"/>
...
import { initScreenshotScheduler, hasActiveScreenshotStream, isFullDesktopCaptureLikely, ensureDisplayStreamAsync, setCageWindowCaptureFallback, setLinuxKioskRunningInCage, setAllowWindowFallbackOnStreamFailure, isCageWindowCaptureFallback } from '../utils/screenshotCapture.js'
...
import { mergeDiscoveredExams } from "../utils/examDiscovery.ts";
...
import { registerWithTeacherCertificate, registrationErrorText } from '../utils/commonMethods.js';
...
export default {
    ...
    setup() {
      let {development, version, serverApiPort, electron, info, buildDate, hostip, bipIntegration, bipApiUrl, bipLoginUrl, demoBipApiUrl, demoBipLoginUrl, bipDemo} = storeToRefs(useConfigStore());
      const username = ref(development.value ? "thomas" : "");
      const pincode = ref(development.value ? "1111" : "");

      let { token } = storeToRefs(useInfoStore());
      return { username, pincode, development, version, serverApiPort, electron, info, buildDate, hostip, bipIntegration, bipApiUrl, bipLoginUrl, demoBipApiUrl, demoBipLoginUrl, bipDemo,
          token};
    },
    ...
    methods: {
        ...
        async loginBiP() {
            ...
            let IPCresponse = window.ipcRenderer.sendSync('loginBiP', isIOS() ? (this.bipDemo ? this.demoBipLoginUrl : this.bipLoginUrl) : this.biptest)
        ...
        logoutBiP() {
            this.$swal({
                ...
                if (result.isConfirmed) {
                    if (!isIOS()) {
                        try {
                            await window.ipcRenderer.invoke('clearBipPortalSession', this.biptest)
                        } catch (e) {
                            console.warn('student @ logoutBiP: clearBipPortalSession', e)
                        }
                    }
                    this.bipToken = false
                    ...
                    if (isIOS()) iosUpdateListener.clearBipSiteInfo()
        ...
        // BiP Anmelden only when portal says open, student is allowed and teacher is reachable on LAN
        isBipExamLanJoinable(server) {
            return !!(server?.bip && !this.isBipExamRestricted(server) && server.examStatus === 'open'
                && server.serverip && server.reachable !== false);
        },

        /** true if the student may not join this bip exam: bip login required but missing, or logged in but exam not assigned to him in the portal */
        isBipExamRestricted(server) {
            if (!server?.bip) { return false }
            if (server.requireBiP) { return !this.bipuserID }
            if (!this.bipuserID) { return false }   // no bip login needed, pin is enough
            const serverKey = server.id ?? server.servername;
            return !this.onlineExams.some(exam => (exam.id ?? exam.examName) === serverKey || exam.examName === server.servername);
        },
        ...
        getBiPUrl(): string {
            const url = this.bipDemo ? this.demoBipApiUrl : this.bipApiUrl;
        ...
        async fetchBipExams() {
            ...
            const url = this.getBiPUrl() + '?wstoken=' + token + '&wsfunction=local_dpu_get_exams_student&moodlewsrestformat=json';
        ...
        fetchBiPData(base64String) {
            ...
            let url = this.getBiPUrl()+'?wstoken='+token+'&wsfunction=core_webservice_get_site_info&moodlewsrestformat=json';
                ...
                .then(async (response) => {
                    if (response.fullname){
                        ...
                        if (response.userprivateaccesskey) {
                            ...
                            if (isIOS()) iosUpdateListener.setBipSiteInfo({
                                userprivateaccesskey: response.userprivateaccesskey,
                                userid: response.userid,
                                fullname: response.fullname,
                            })
        ...
        async fetchInfo() {
            ...
            if (this.advanced && !this.token) {
                if (this.serverip !== "") {
                    if (validator.isIP(this.serverip) || validator.isFQDN(this.serverip)) {
                        ...
                        window.ipcRenderer.invoke('fetch-exam-server-list', { serverip: this.serverip })
                            .then(data => {
            ...
            if (getinfo.serverlist.length !== 0) {
                ...
                newServerlist = mergeDiscoveredExams(newServerlist);
            ...
            for (let server of this.serverlist) {
                ...
                window.ipcRenderer.invoke('pingexamserver', { serverip: server.serverip })
                    .then((result) => {
                        // Teacher reachable but serving a new certificate: keep the reason for the tooltip.
                        server.certChanged = result?.status === 'fingerprint-changed';
                        if (!result?.ok) throw Object.assign(new Error('Response not OK'), { handled: true });
                    ...
                    .catch(err => {
                        ...
                        // The ping itself failed, so no certificate verdict exists for this round.
                        if (!err.handled) { server.certChanged = false; }
                        // Optimized: Only set if value changes
        ...
        async registerClient(serverip, servername) {
            ...
            // Win32 AssignedAccess and plain desktop use getDisplayMedia. iOS has no screenshotCapture
            // scheduler at all (initScreenshotScheduler skips non-Electron), so these checks never apply there.
            if (isElectronWindow() && !isCageWindowCaptureFallback()) {
            ...
            const charMap = {
                ...
                const IPCresponse = await registerWithTeacherCertificate(this, {
                ...
                if (IPCresponse && IPCresponse.status === "success") {
                    // Registration proves the API is reachable; the ping loop stops once a token exists,
                    // so clear a stale unreachable mark here (e.g. after the Teacher restarted).
                    const registeredServer = this.serverlist.find(s => s.serverip === serverip)
                    if (registeredServer) { registeredServer.reachable = true }
                ...
                if (IPCresponse && IPCresponse.status === "error") {
                    this.$swal.fire({
                        title: "Error",
                        text: registrationErrorText(this, IPCresponse),
                        icon: 'error',
                        showCancelButton: false,
                    })
                }
    ...
    async mounted() {
      if (isIOS()) {
        ...
        if (isElectronWindow(window)) {
            ...
            // Broken KWin/portal Wayland: getDisplayMedia fails → same capturePage fallback (window-only).
            setAllowWindowFallbackOnStreamFailure(this.platformKiosk.platform === 'linux' && this.platformKiosk.displayServer === 'wayland');
      ...
      // add event listener to user input field to supress all special 1chars
        ...
        window.ipcRenderer.on('bipToken', (event, token) => {
            // todo: fix the whole event / payload thing discrepancy betweeen ios and electron
            const bipToken = isIOS() ? event : token
            this.bipToken = bipToken
            this.fetchBiPData(bipToken)
        });

        window.ipcRenderer.on('bip-user-id', (_event, bipUserId) => {
            useInfoStore().bipUserId = bipUserId
        });
    ...
    beforeUnmount() {
        ...
        window.ipcRenderer.removeAllListeners('bip-user-id');
