 <template> 
    ...
    <webview v-if="!isIOSPlatform" id="webviewmain" autosize="on" :src="url" :style="{ visibility: isLoading ? 'hidden' : 'visible' }"></webview>
    <div v-else id="webviewmain"></div>
...
export default {
    ...
    setup() {
        let {development, serverApiPort, hostip, showdevtools} = storeToRefs(useConfigStore());

        const infoStore = useInfoStore();
        infoStore.online = true;
        infoStore.componentName = "Lernplattform";
        let {examtype, clientname, serverstatus, localLockdown, battery, wlanInfo, entrytime} = storeToRefs(infoStore);

        return {isIOSPlatform: isIOS(), 
          development, serverApiPort, hostip, showdevtools,
    ...
    data() {
        return {
            ...
            _onMoodleLoaded: null,
            _onMoodleLoadFailed: null,
    ...
    mounted() {
        if (isIOS()) {
            this._onMoodleLoaded = () => { this.isLoading = false; };
            this._onMoodleLoadFailed = (error) => {
                this.isLoading = false;
                console.error('eduvidual @ iOS Moodle load failed:', error);
            };
            window.ipcRenderer.on('moodle-loaded', this._onMoodleLoaded);
            window.ipcRenderer.on('moodle-load-failed', this._onMoodleLoadFailed);
        }
    ...
    methods: { 
        ...
        reloadWebview(){
            if (isIOS()) {
                this.isLoading = true;
                window.ipcRenderer.invoke('reload-moodle');
                return;
            }
            const webview = document.getElementById('webviewmain');
        ...
        // Opens the native iOS Moodle view inside the Vue placeholder.
        async openNativeMoodle() {
            if (!isIOS() || !this.url) return;
            await this.$nextTick();
            const placeholder = document.getElementById('webviewmain');
            if (!placeholder) return;
            const rect = placeholder.getBoundingClientRect();
            this.isLoading = true;
            await window.ipcRenderer.invoke('open-moodle', {
                url: this.url,
    ...
    beforeUnmount() {
        ...
        if (isIOS()) {
            window.ipcRenderer.removeAllListeners('moodle-loaded');
            window.ipcRenderer.removeAllListeners('moodle-load-failed');
            window.ipcRenderer.invoke('close-moodle');
        }
