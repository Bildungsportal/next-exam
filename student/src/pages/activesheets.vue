<template>
    ...
    <div id="statusbar" style="padding-left:15px;">
        <img @click="zoomin();" src="/img/svg/zoom-in.svg" class="zoombutton">
        <img @click="zoomout();" src="/img/svg/zoom-out.svg" class="zoombutton">
    </div>
    </div>
</template>
...
export default {
    ...
    setup() {
        let {development, serverApiPort, hostip} = storeToRefs(useConfigStore());

        const infoStore = useInfoStore();
        let {examtype, servername, serverip, token, clientname, serverstatus, localLockdown, battery, wlanInfo, entrytime} = storeToRefs(infoStore);
    ...
    methods: { 
        ...
        async saveSectionSwitchBackup() {
            ...
            if (isIOS()) {
                const result = await iosUpdateListener.storeHTML(this.clientname, JSON.stringify(formData, null, 2), 'sectionswitch');
                return result?.returnSignal === 'loadfilelist';
            }
        ...
        async loadBackupFileSilent(filename = false) {
            ...
            try {
                const [backupfileContent, ready] = await Promise.all([
                    isIOS() ? iosUpdateListener.getBackupFile(backupfileName) : window.ipcRenderer.invoke('getbackupfile', backupfileName),
        ...
        async loadBackupFile(filename=false){
            ...
            try {
                let backupfileContent = isIOS()
                    ? await iosUpdateListener.getBackupFile(backupfileName)
                    : await window.ipcRenderer.invoke('getbackupfile', backupfileName)
        ...
        async loadBAK(filename, skipDialog = false, silent = false) {
            try {
                const bakContent = isIOS()
                    ? await iosUpdateListener.getBackupFile(filename)
                    : await window.ipcRenderer.invoke('getbackupfile', filename);
        ...
        async saveContent(backup, why) {
            ...
            if (isIOS()) {
                await iosUpdateListener.storeHTML(filename || this.clientname, JSON.stringify(formData, null, 2), why)
            } else {
            ...
            await this.withActivesheetsPdfCapture(async () => {
                const args = this.activesheetsPdfInvokeArgs(why)
                const response = isIOS()
                    ? await iosUpdateListener.getBase64PDF(args, why)
                    : await window.ipcRenderer.invoke('getPDFbase64', args)
                if (response?.status === 'success') {
                    if (isIOS()) {
                        await iosUpdateListener.storePDF(filename || this.clientname, response.base64pdf, why)
                    } else {
        ...
        async sendExamToTeacher(directsend=false, type="send"){
            ...
            await this.withActivesheetsPdfCapture(async () => {
                const args = this.activesheetsPdfInvokeArgs(reason)
                const response = isIOS()
                    ? await iosUpdateListener.getBase64PDF(args, reason)
                    : await window.ipcRenderer.invoke('getPDFbase64', args)
                if (response?.status !== 'success') {
                    this.showPdfGenerationError(response)
                    return
    ...
    mounted() {
        ...
        this.$nextTick(async () => { 
            ...
            window.ipcRenderer.on('save', (event, why) => {  //trigger document save by signal "save" sent from sendExamtoteacher in communication handler
                ...
                this.saveContent(true, typeof why === 'string' ? why : event)
            });
