...
private struct StudentPage {
    ...
    func element(label: String, timeout: TimeInterval = 60) -> XCUIElement {
        ...
        XCTAssertTrue(element.waitForExistence(timeout: timeout), "Missing accessibility element: \(label)")
        return element
    }
...
final class AppUITests: XCTestCase {
    ...
    func testStudentCompletesSprachenExam() {
        ...
        XCTAssertTrue(app.wait(for: .runningForeground, timeout: 60), "App did not enter foreground")
        app.tap()
        XCTAssertTrue(app.webViews.firstMatch.waitForExistence(timeout: 60), "Capacitor web view did not appear")
        let student = StudentPage(app: app)
        let pin = ProcessInfo.processInfo.environment["NXE_E2E_PIN"] ?? ""
        XCTAssertTrue(pin.range(of: #"^\d{4}$"#, options: .regularExpression) != nil, "NXE_E2E_PIN is invalid: \(pin)")
        student.register(name: "iosstudent", pinCode: pin, examName: "e2e-ios")
