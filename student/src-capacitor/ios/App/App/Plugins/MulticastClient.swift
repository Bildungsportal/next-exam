...
struct AppConfig {
    ...
    static let version: String             = "2.0.0.7"
}
...
public final class MulticastClient: CAPPlugin, CAPBridgedPlugin {
    ...
    public override func load() {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(networkInterfaceAvailable),
            name: Notification.Name("NetworkInterfaceAvailable"),
            object: nil
        )
        if !Config.hostip.isEmpty { startMulticast() }
        ...
        IPCBridge.shared.handle("pingexamserver") { [weak self] payload async throws -> Any? in
            ...
            TeacherCertificateTrust.shared.allow(serverip)
            do {
                ...
                NextExamApiAuthentication.apply(to: &request)
        ...
        // Fetch Teacher exams directly when multicast discovery is unavailable.
        IPCBridge.shared.handle("fetch-exam-server-list") { payload async throws -> Any? in
            guard let dict = payload as? [String: Any],
                  let serverip = dict["serverip"] as? String,
                  !serverip.isEmpty else {
                return ["status": "error"]
            ...
            TeacherCertificateTrust.shared.allow(serverip)
            do {
                guard let url = URL(string: "https://\(serverip):\(Config.serverApiPort)/server/control/serverlist") else {
                    return ["status": "error"]
                }
                var request = URLRequest(url: url, timeoutInterval: 4.0)
                request.httpMethod = "GET"
                NextExamApiAuthentication.apply(to: &request)
                let session = URLSession(configuration: .default, delegate: LocalNetworkSessionDelegate(), delegateQueue: nil)
                let (data, _) = try await session.data(for: request)
                return try JSONSerialization.jsonObject(with: data)
            } catch {
                if TeacherCertificateTrust.shared.hasPending(serverip) {
                    return ["status": "fingerprint-changed", "serverip": serverip]
                }
                return ["status": "error"]
            }
        }
        ...
        IPCBridge.shared.handle("accept-teacher-certificate") { payload async throws -> Any? in
            guard let args = payload as? [String: Any],
                  let serverip = args["serverip"] as? String else {
                return ["ok": false]
            }
            return ["ok": TeacherCertificateTrust.shared.acceptPending(serverip)]
        }
    ...
    private func register(args: [String: Any]) async throws -> [String: Any] {
        ...
        do {
            ...
            if response.status == "success" {
                ...
                self.reconnectToken        = response.token
                ...
                return ["status": "error", "message": response.message ?? "Unknown error", "code": response.code ?? ""]
            }
    ...
    /// Starts multicast after NetworkPlugin has selected an active IPv4 interface.
    @objc private func networkInterfaceAvailable() {
        startMulticast()
    }
    ...
    private func startMulticast() {
        ...
        mreq.imr_interface.s_addr = inet_addr(Config.hostip)
    ...
    private func stopMulticast(interfaceAddr: String? = nil) {
        ...
        var mreq                  = ip_mreq()
        mreq.imr_multiaddr.s_addr = inet_addr(AppConfig.multicastServerAddr)
        let iface = interfaceAddr ?? Config.hostip
    ...
    private func messageReceived(data: Data, senderIP: String, senderPort: Int) {
        ...
        TeacherCertificateTrust.shared.allow(senderIP, fingerprint: info.tlsFingerprint)
