...
async function startExam(teacherWindow, studentWindow, locatorString, urlSubstring) {
  await teacherWindow.locator('.control-buttons-container .btn-teal.control-button').first().click();
  await expect(studentWindow.locator(locatorString)).toBeVisible({ timeout: 60_000 });
  await expect.poll(() => studentWindow.url()).toContain(urlSubstring);
}
...
const modes = [
  {
    ...
    run: async ({ teacherWindow, studentWindow, studentWidget }) => {
      ...
      await test.step('student generates the final preview and submits the exam', async () => {
        await studentWindow.locator('#sendfinalexam').click();
        const sendButton = studentWindow.locator('#send-button');
  ...
  }, 
  {
    ...
    run: async ({ teacherWindow, studentWindow, examName, studentName }) => {
      ...
      await test.step('teacher collects the saved GeoGebra file', async () => {
        await teacherWindow.locator('.tab-buttons-container .tab-button').nth(1).click();
  ...
  },
  {
    ...
    run: async ({ teacherWindow, studentWindow }) => {
      await test.step('teacher starts the exam and student receives the website', async () => {
        await startExam(teacherWindow, studentWindow, '#webviewmain', '/website/');
        const webview = studentWindow.locator('#webviewmain');
        await expect(webview).toHaveAttribute('src', 'https://example.com/', { timeout: 60_000 });
      });
    }
  },
  {
    ...
    test('student completes the exam flow', async () => {
      ...
      try {
        ...
        await test.step(`teacher creates an ${mode.name} exam`, async () => {
          ...
          await teacherWindow.locator('.sidebar-exammode-dropdown-wrap .dropdown-item').nth(mode.modeIndex).click();
          await mode.configure?.({ teacherWindow });
          await addCustomResources(teacherWindow, mode.resources);
        });

        await test.step('student discovers the exam and registers', async () => {
          examPin = (await teacherWindow.locator('.sidebar-info-strip > .infobutton').nth(1).locator('b').textContent()).trim();
        ...
        await mode.run({ teacherWindow, studentWindow, studentWidget, examName, studentName });

        await test.step('teacher ends the exam', async () => {
          await teacherWindow.locator('.btn.btn-sm.btn-danger.m-0.me-1.mt-0').first().evaluate((element) => element.click());
          await teacherWindow.locator('.swal2-confirm.swal2-styled.swal2-default-outline').first().click();
        });
      } finally {
