...
async function kick(studenttoken, studentip){
    ...
    const offline = !isStudentReachable(studentRow, Date.now())   // disconnected student: no files can be deleted on his pc anymore

    let freshSubmissions = Array.isArray(this.submissions) ? this.submissions : []
    const warnSubmissions = !offline && activeSectionUsesAbgabeSubmissionWarnings(this.serverstatus)
    if (warnSubmissions) {
        try {
            if (typeof window !== 'undefined' && window.ipcRenderer?.invoke) {
                freshSubmissions = await window.ipcRenderer.invoke('getSubmissions', this.servername, JSON.stringify(this.serverstatus))
    ...
    this.$swal.fire({
        ...
        title: this.$t("dashboard.sure"),
        html: offline ? `<div class="my-content">
        <span style='font-weight:bold;'>${studentname}</span> ${this.$t("dashboard.reallykickoffline")}
        </div>
...
//remove all disconnected students from exam (network outage cleanup)
function kickAllOffline(){
    const now = Date.now()
    const offlineStudents = this.studentlist.filter(student => !isStudentReachable(student, now))
    if (offlineStudents.length <= 0) { this.status(this.$t("dashboard.noclients")); return; }
    ...
    this.$swal.fire({
        ...
        title: this.$t("dashboard.sure"),
        html: `<div class="my-content">
        ${this.$t("dashboard.reallykickalloffline", { count: offlineStudents.length })}
        </div>
        `,
        icon: "error",
        showCancelButton: true,
    ...
    .then((result) => {
        ...
        for (const student of offlineStudents) {
            examEventBus.push('kick', student)
            ipcRenderer.invoke('setStudentStatus', {
                servername: this.servername,
                ...
                .then((res) => { log.info('exammanagment @ kickAllOffline:', res.message) })
        }
    });
}
...
export {activateSpellcheckForStudent, delfolderquestion, stopserver, sendFiles, lockscreens, getFiles, startExam, lockSectionForAll, endExam, kick, kickAllOffline, restore  }
