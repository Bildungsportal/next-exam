    ...
    const result = await this.$swal.fire({
        ...
        html: `
            <div class="my-content" style="font-size: 1em;">
                ...
                <div id="urlConfig" >
                    <div style="justify-content: flex-start" class="swal2-input-label">${this.$t("dashboard.eduvidualTestUrlHint")}</div>
...
/** VM name for a new install; default = next free win11-N. null when cancelled. */
async function promptNewVmName(vm, ipc) {
    let suggested = '';
    try { suggested = await ipc.invoke('qemu-suggest-disk-name'); } catch (e) {}
    const res = await vm.$swal.fire({
        title: vm.$t('dashboard.localvmNewVmNameTitle'),
        text: vm.$t('dashboard.localvmNewVmNameLabel'),
        icon: 'question',
        input: 'text',
        inputValue: suggested || '',
        showCancelButton: true,
        confirmButtonText: 'Ok',
        cancelButtonText: vm.$t('dashboard.cancel'),
        inputValidator: (value) => {
            const name = String(value || '').trim().replace(/\.qcow2$/i, '');
            if (!name) return vm.$t('dashboard.localvmNewVmNameRequired');
            if (!/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(name)) {
                return vm.$t('dashboard.localvmNewVmNameInvalid');
            }
        },
    });
    if (!res.isConfirmed) return null;
    return String(res.value || '').trim().replace(/\.qcow2$/i, '');
}
...
async function configureLocalVM(presetGroup){
    ...
    if (!Array.isArray(disks) || disks.length === 0) {
        ...
        const firstHtml = `<div style="text-align:left;">
            ...
            <div style="margin-top:10px; display:flex; gap:8px; flex-wrap:wrap;">
                <button type="button" class="btn btn-sm btn-cyan" id="qemuInstallBtn">${escapeSwalHtmlText(this.$t('dashboard.localvmInstallDefaultBtn'))}</button>
                <button type="button" class="btn btn-sm btn-cyan" id="qemuInstallIsoBtn">${escapeSwalHtmlText(this.$t('dashboard.localvmInstallFromIsoBtn'))}</button>
        ...
        const result = await this.$swal.fire({
            ...
            didOpen: () => {
                ...
                const installIsoBtn = document.getElementById('qemuInstallIsoBtn');
                ...
                const runVmInstall = async (isoPath = null) => {
                    if (!(await ensureQemuAvailableForLocalVm(this))) {
                        return;
                    }
                    const diskName = await promptNewVmName(this, ipc);
                    if (!diskName) {
                        setTimeout(() => { configureLocalVM.call(this, presetGroup); }, 50);
                        return;
                    }
                    this.$swal.fire({
                        title: 'LocalVM',
                        html: `<div id="qemuHashStatus" style="margin-top:10px; font-size:0.9em; color:#6c757d;">Starte VM-Build…</div>`,
                        allowOutsideClick: false,
                    ...
                    try {
                        onProgress = (_event, payload) => {
                            ...
                            if (phase === 'resolving-iso-url') {
                                el.textContent = 'VM-Build: ermittle Windows-ISO Download-Link…';
                                return;
                            }
                    ...
                    try {
                        const res = await ipc.invoke('qemu-install-default', { isoPath, diskName });
                        if (res?.qemuMissing) {
                        ...
                        if (!res || res.ok !== true) {
                            await this.$swal.fire({
                                icon: 'error',
                                title: 'LocalVM',
                                text: `${this.$t('dashboard.localvmVmBuildFailed', { error: res?.error || this.$t('dashboard.localvmUnknownError') })}${isoPath ? '' : ' ' + this.$t('dashboard.localvmManualIsoHint')}`,
                            });
                            return;
                        }
                    } catch (e) {
                        await this.$swal.fire({
                            icon: 'error',
                            title: 'LocalVM',
                            text: `${this.$t('dashboard.localvmVmBuildFailed', { error: String(e?.message || e) })}${isoPath ? '' : ' ' + this.$t('dashboard.localvmManualIsoHint')}`,
                        });
                        return;
                    } finally {
                        try {
                ...
                installBtn?.addEventListener('click', () => { runVmInstall(); });
                installIsoBtn?.addEventListener('click', async () => {
                    const pickIso = await ipc.invoke('qemu-pick-install-iso');
                    if (pickIso?.cancelled) return;
                    if (!pickIso?.ok || !pickIso.sourcePath) {
                        if (statusEl) statusEl.textContent = pickIso?.error ? String(pickIso.error) : 'ISO-Auswahl fehlgeschlagen.';
                        return;
                    }
                    await runVmInstall(pickIso.sourcePath);
                });
            },
        });
    ...
    const html = `<div class="my-content" style="text-align:left; padding:0px!important;">
        ...
        <div style="padding:0; border:1px solid rgba(255,255,255,0.08); border-radius:8px; background:rgba(255,255,255,0.03);">
            <div style="font-weight:700; margin-bottom:8px;">${escapeSwalHtmlText(this.$t('dashboard.localvmAdvanced'))}</div>
            <div style="display:flex; align-items:center; justify-content:space-between; gap:10px;">
                <label for="qemuBlockInternet" style="margin:0; font-size:0.85em;">${escapeSwalHtmlText(this.$t('dashboard.localvmBlockInternet'))}</label>
            ...
            <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; margin-top:8px;">
                <label for="qemuCalculateSha256" style="margin:0; font-size:0.85em;">
                    ${escapeSwalHtmlText(this.$t('dashboard.localvmSha256Check'))}
                    <span
                        title="${escapeSwalHtmlText(this.$t('dashboard.localvmSha256CheckInfo'))}"
        ...
        <div style="padding:0; border:1px solid rgba(255,255,255,0.08); border-radius:8px; background:rgba(255,255,255,0.03);">
            <div style="font-weight:700; margin-bottom:8px;">${escapeSwalHtmlText(this.$t('dashboard.localvmActions'))}</div>
            <div style="display:flex; gap:8px; flex-wrap:wrap;">
                <button type="button" class="btn btn-sm btn-cyan" id="qemuBrowseBtn">${escapeSwalHtmlText(this.$t('dashboard.localvmBrowseBtn'))}</button>
                <button type="button" class="btn btn-sm btn-cyan" id="qemuInstallBtn">${escapeSwalHtmlText(this.$t('dashboard.localvmInstallBtn'))}</button>
                <button type="button" class="btn btn-sm btn-cyan" id="qemuInstallIsoBtn">${escapeSwalHtmlText(this.$t('dashboard.localvmInstallIsoBtn'))}</button>
            </div>
    ...
    const pick = await this.$swal.fire({
        ...
        title: this.$t('dashboard.localvmGroupTitle', { group: hasGroups ? activeGroup.toUpperCase() : 'AB' }),
        icon: 'question',
        html,
        showCancelButton: true,
        cancelButtonText: this.$t('dashboard.cancel'),
            ...
            const runVmInstall = async (isoPath = null) => {
                ...
                try {
                    onProgress = (_event, payload) => {
                        ...
                        if (phase === 'resolving-iso-url') {
                            statusEl.textContent = 'VM-Build: ermittle Windows-ISO Download-Link…';
                            return;
                        }
                        if (phase === 'creating-disk') {
...
/** Patch group.examConfig.math for the active section (mirrors both groups, like editor). */
function setMathExamConfigPatch(patch) {
    const section = this.serverstatus.examSections[this.serverstatus.activeSection];
    if (!section) return;

    const { groupA, groupB } = ensureGroupsAndExamConfig(section);
    const next = { ...(groupA.examConfig.math || {}), ...patch };

    groupA.examConfig.math = next;
    groupB.examConfig.math = { ...next };
...
function extractDomainAndId(url) {
    ...
    var testid = null;
    // lms.at: quiz/assignment id is the last path segment (not ?id=)
    if (moodledomain === 'lms.at') {
        var pathOnly = String(url).replace(/^(https?:\/\/)?[^\/]+/i, '').split(/[?#]/)[0];
        var segs = pathOnly.replace(/\/+$/, '').split('/').filter(Boolean);
        testid = segs.length ? segs[segs.length - 1] : null;
    } else {
        var idMatch = url.match(/id=(\d+)/);
        testid = idMatch ? idMatch[1] : null;
    }
