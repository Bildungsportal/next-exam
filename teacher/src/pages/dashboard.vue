...
<div :key="0" class="w-100 p-3 text-white bg-dark text-left " style="position: relative; display: flex; align-items: center; justify-content: space-between; min-width: 1180px; height: 62px; z-index: 100;">
    ...
    <div style="flex-shrink: 0; display: flex; align-items: center; justify-content: flex-end; flex-wrap: wrap;">
        <div class="btn btn-sm btn-danger m-0 me-1 mt-0" data-e2e="stop-server" @click="stopserver()" @mouseover="showDescription($t('dashboard.exitexam'))" @mouseout="hideDescription"  style=" height:32px;"><img src="/src/assets/img/svg/stock_exit.svg" style="vertical-align:text-top;" class="" width="20" height="20" >&nbsp; {{$t('dashboard.stopserver')}}&nbsp; </div>
    ...
    <div v-if="serverstatus.useExamSections && !serverstatus.allowSectionSwitch" style="position: absolute; left:256px; bottom: 0; min-width: 550px; z-index: 0;">
        <div id="section1" v-if="serverstatus.examSections[1]" @click="activateSection(1)" class="sectionbutton btn btn-sm" :class="{'sectionbuttonactive': serverstatus.activeSection == 1 && !serverstatus.examSections[1].locked, 'sectionbuttonactivered': serverstatus.activeSection == 1 && serverstatus.examSections[1].locked, 'btn-secondary': serverstatus.activeSection != 1,'btn-danger': serverstatus.examSections[1].locked}">
            <span class="sectionbutton-label" :class="{ 'sectionbutton-label-disabled': serverstatus.examSections[1].sectiondisabled }">{{ serverstatus.examSections[1].sectionname }}</span>
        ...
        <div id="section2" v-if="serverstatus.examSections[2]" @click="activateSection(2)" class="sectionbutton btn btn-sm" :class="{'sectionbuttonactive': serverstatus.activeSection == 2 && !serverstatus.examSections[2].locked, 'sectionbuttonactivered': serverstatus.activeSection == 2 && serverstatus.examSections[2].locked, 'btn-secondary': serverstatus.activeSection != 2,'btn-danger': serverstatus.examSections[2].locked}">
            <span class="sectionbutton-label" :class="{ 'sectionbutton-label-disabled': serverstatus.examSections[2].sectiondisabled }">{{ serverstatus.examSections[2].sectionname }}</span>
        ...
        <div id="section3" v-if="serverstatus.examSections[3]" @click="activateSection(3)" class="sectionbutton btn btn-sm" :class="{'sectionbuttonactive': serverstatus.activeSection == 3 && !serverstatus.examSections[3].locked, 'sectionbuttonactivered': serverstatus.activeSection == 3 && serverstatus.examSections[3].locked, 'btn-secondary': serverstatus.activeSection != 3,'btn-danger': serverstatus.examSections[3].locked}">
            <span class="sectionbutton-label" :class="{ 'sectionbutton-label-disabled': serverstatus.examSections[3].sectiondisabled }">{{ serverstatus.examSections[3].sectionname }}</span>
        ...
        <div id="section4" v-if="serverstatus.examSections[4]" @click="activateSection(4)" class="sectionbutton btn btn-sm" :class="{'sectionbuttonactive': serverstatus.activeSection == 4 && !serverstatus.examSections[4].locked, 'sectionbuttonactivered': serverstatus.activeSection == 4 && serverstatus.examSections[4].locked, 'btn-secondary': serverstatus.activeSection != 4,'btn-danger': serverstatus.examSections[4].locked}">
            <span class="sectionbutton-label" :class="{ 'sectionbutton-label-disabled': serverstatus.examSections[4].sectiondisabled }">{{ serverstatus.examSections[4].sectionname }}</span>
...
<div id="wrapper" class="w-100 h-100 d-flex"  style="z-index: 100;">
    ...
    <div :key="5" class=" text-white bg-dark d-flex flex-column sidebar-root mt-3" style="width: 240px; min-width: 240px;">
        ...
        <div class="text-start infobutton d-flex align-items-start">
            <div class="flex-grow-1 min-w-0">{{$t('dashboard.pin')}}<br><b data-e2e="exam-pin"> {{ serverstatus.pin }} </b></div>
        ...
        <div v-if="!serverstatus.examSections[serverstatus.activeSection]?.sectiondisabled" class="sidebar-scroll" :class="{ 'sidebar-groups-inactive': !serverstatus.examSections[serverstatus.activeSection]?.groups }">
        <div class="dropdown-section" :class="lockInExammode ? 'disabledexam-dropdown' : ''">
            ...
            <button class="btn btn-sm btn-secondary dropdown-toggle d-flex justify-content-between align-items-center sidebar-exammode-toggle w-100" data-e2e="exam-mode-toggle" style="text-align: left;" type="button" data-bs-toggle="dropdown" aria-expanded="false" data-bs-popper="static">
                 <span>{{ getSelectedExamTypeLabel() }}</span>
            </button>
            <ul class="dropdown-menu" style="cursor: pointer;">
                <li v-if="config.exammodes && config.exammodes.math"><a class="dropdown-item" data-e2e="exam-mode-math" @click="selectExamType('math')" :class="{ active: isExamType('math') }">{{$t('dashboard.math')}}</a></li>
                <li v-if="config.exammodes && config.exammodes.editor"><a class="dropdown-item" data-e2e="exam-mode-editor" @click="selectExamType('editor')" :class="{ active: isExamType('editor') }">{{$t('dashboard.lang')}}</a></li>
                <li v-if="config.exammodes && config.exammodes.activesheets"><a class="dropdown-item" data-e2e="exam-mode-activesheets" @click="selectExamType('activesheets')" :class="{ active: isExamType('activesheets') }">Active Sheets</a></li>
                ...
                <li v-if="config.exammodes && config.exammodes.website"><a class="dropdown-item" data-e2e="exam-mode-website" @click="selectExamType('website')" :class="{ active: isExamType('website') }">Website</a></li>
            ...
            <div class="mt-2">
                ...
                <div v-if="isExamType('math')" class="basematerial-sidebar-block basematerial-sidebar-block--optional mt-3">
                    <div class="basematerial-panel-caption">{{ $t('dashboard.geogebraSettings') }}</div>
                    <div class="form-check form-switch m-0">
                        <input id="sidebar-math-enable3d"
                               class="form-check-input"
                               type="checkbox"
                               :checked="serverstatus.examSections[serverstatus.activeSection].groupA?.examConfig?.math?.enable3D !== false"
                               @change="setMathExamConfigPatch({ enable3D: $event.target.checked })">
                        <label class="form-check-label" for="sidebar-math-enable3d">{{ $t('dashboard.enable3d') }}</label>
                    </div>
                </div>
                            ...
                            <div class="setup-row">
                                ...
                                <div class="setup-inline setup-inline-fill">
                                    <input id="screenshotIntervalSlider" type="range"
                                        v-model.number="serverstatus.screenshotinterval"
                                        :min="0" :max="60" step="2"
                            ...
                            <div class="setup-row">
                                ...
                                <div class="setup-inline setup-inline-fill">
                                    <input id="backupintervalSlider" type="range"
                                        v-model.number="serverstatus.backupintervalPause"
                                        :min="0" :max="20" step="1"
                                        class="form-range custom-slider setup-range"
    ...
    <div :key="7" id="content" class="fadeinslow p-3">
       ...
       <div class="control-buttons-container">
        <div v-if="(serverstatus.exammode && reachableConnections == 1)" class="btn btn-danger control-button m-1 mt-0 text-start ms-0" data-e2e="end-exam" @click="endExam();hideDescription();" @mouseover="showDescription($t('dashboard.exitkiosk'))" @mouseout="hideDescription">
        ...
        <div v-if="(!serverstatus.exammode && reachableConnections == 1)"
            @mouseover="showDescription(hasMandatoryBasematerialReady ? $t('dashboard.startexamdesc') : $t('dashboard.mandatoryBasematerialIncompleteHint'))"
            @mouseout="hideDescription">
            <div class="btn btn-teal control-button m-1 mt-0 text-start ms-0" data-e2e="start-exam" @click="startExam();hideDescription();" :class="!hasMandatoryBasematerialReady ? 'disabledgreen':''">
                <img src="/src/assets/img/svg/shield-lock.svg" class="white control-button-icon" width="28" height="28"> 
        ...
        <div class="tab-buttons-container">
            <div v-if="hasOfflineStudents"
                class="btn tab-button tab-button-offline"
                @click="kickAllOffline();hideDescription();"
                @mouseover="showDescription($t('dashboard.kickalloffline'))"
                @mouseout="hideDescription">
                <img src="/src/assets/img/svg/list-remove-user.svg" width="24" height="24">
            </div>
            ...
            <div class="btn btn-dark tab-button" data-e2e="collect-files"
        ...
        <!-- LOG START -->
        <div id="loginfo">
            <div id="logcheck" :class="{ 'logcheck-offline': hasOfflineStudents }" @click="fetchLOG();" @mouseover="showDescription($t('dashboard.serverlogdesc'))" @mouseout="hideDescription"> <div id="eye" class="darkgreen eyeopen"></div> &nbsp;Server Log</div>
...
export default {
    ...
    data() {
        return {
            ...
            bipPhase: "ready",
            biptest:this.$route.params.biptest === true || this.$route.params.biptest === 'true',   // params are always passed as string, convert to bool
...
computed: {
    // true while at least one registered student is disconnected (shows the bulk-remove sidebar button)
    hasOfflineStudents() { return this.studentlist.some(s => !isStudentReachable(s, this.now)) },
    ...
    methods: {
        ...
        async editSectionName(sectionIndex) {
            ...
            const result = await this.$swal.fire({
                ...
                html: `
                    ...
                    <div class="form-check form-switch mt-3 text-start">
                        <input id="nx-section-disabled" class="form-check-input" type="checkbox" ${section.sectiondisabled ? 'checked' : ''}>
                        <label class="form-check-label" for="nx-section-disabled">${this.$t('dashboard.sectiondisabled')}</label>
                    </div>
                `,
                ...
                preConfirm: () => {
                    ...
                    const disabledEl = document.getElementById('nx-section-disabled');
                    const nextDisabled = !!disabledEl?.checked;
                    ...
                    return { nextName, nextGroups, nextTimelimit, nextDisabled };
                },
            });

            if (!result.isConfirmed) return;
            const { nextName, nextGroups, nextTimelimit, nextDisabled } = result.value || {};
            if (!nextName) return;

            const groupsChanged = !!section.groups !== !!nextGroups;
            section.sectiondisabled = !!nextDisabled;
        ...
        kickAllOffline: kickAllOffline,              //remove all disconnected students from exam
        ...
        setMathExamConfigPatch: setMathExamConfigPatch,
        ...
        onToggleExamSections(){
            if (!this.serverstatus.useExamSections) {
                ...
                // Copy connected students into every section groupA (skip if already in A/B)
                for (const section of Object.values(this.serverstatus.examSections)) {
                    if (!section?.groupA) continue
                    if (!Array.isArray(section.groupA.users)) section.groupA.users = []
                    const groupB = section.groupB?.users || []
                    for (const student of this.studentlist) {
                        const name = student.clientname
                        if (!name) continue
                        const lc = String(name).toLowerCase()
                        const inA = section.groupA.users.some((u) => String(u).toLowerCase() === lc)
                        const inB = groupB.some((u) => String(u).toLowerCase() === lc)
                        if (!inA && !inB) section.groupA.users.push(name)
                    }
                }
            }
            this.setServerStatus()
        ...
        activateSection(section){
            ...
            // Disabled section: dashboard config only — never offer lock/activate for students
            if (this.serverstatus.examSections[section]?.sectiondisabled) return
        ...
        updateBackupInterval() {
            const interval = this.serverstatus.backupintervalPause;
            if (interval === 0) {
        ...
        applyTeacherGlobalSettings(global) {
            ...
            if (global.screenshotinterval != null) this.serverstatus.screenshotinterval = parseInt(global.screenshotinterval, 10) || 0;
        ...
        migrateServerStatus() {
            ...
            for (const section of Object.values(status.examSections)) {
                ...
                if (typeof section.sectiondisabled !== 'boolean') section.sectiondisabled = false;
        ...
        */
        ...
        getBiPUrl(): string {
            const url = this.config.bipDemo ? this.config.demoBipApiUrl : this.config.bipApiUrl;
            return this.biptest ? url.replace('//www.', '//q.') : url;
        },
        ...
        async updateBiPServerInfo(status){
            ...
            const url= this.getBiPUrl()+"?wstoken="+token+"&wsfunction=local_dpu_update_exam_status_teacher&moodlewsrestformat=json"
    ...
    async mounted() {  // when ready
        this.$nextTick( async function () { // Code that will run only after the entire view has been rendered
            ...
            if (this.serverstatus.backupintervalPause != 0) this.backupinterval.start();
...
/* bulk-remove button for disconnected students - same light red as the offline studentwidgets */
.tab-button-offline {
...
/* the extra offline button adds a row to the tab stack - push the server log tab down by that row */
#logcheck.logcheck-offline {
    margin-top: 360px;
}
