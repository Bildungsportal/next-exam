...
/** Parses submission PDF for the editor's total Zeichen count (last occurrence; '~' when absent, e.g. activesheets) */
        ...
        chars = await pdfParse(dataBuffer)
            .then((data) => {
                if (data && data.text && studentname) {
                    // editor status bar prints "<words> Wörter, <chars> Zeichen" last; statsRule badges use the same wording earlier in the text
                    const matches = [...data.text.matchAll(/(\d+)\s*(?:Zeichen|characters)/g)]
                    if (matches.length > 0) {
...
async function createIndexPDF(submissions, servername, mcServer) {
    ...
    const drawRow = (row, yPos) => {
        row.forEach((cellText, columnIndex) => {
            ...
            page.drawRectangle({ x: xPos, y: yPos - rowHeight, width: columnWidths[columnIndex], height: rowHeight, borderColor: rgb(0, 0, 0), borderWidth: 1 })
            try {
                page.drawText(String(cellText), { x: xPos + 3, y: yPos - rowHeight + 4, size: 9, color: rgb(0, 0, 0) })
            } catch (err) {
                // standard font is WinAnsi only - names with e.g. cyrillic/CJK glyphs would throw and kill the whole index
                log.warn(`getLatestCombinedPdf @ createIndexPDF: cell not printable (${err})`)
                page.drawText('~', { x: xPos + 3, y: yPos - rowHeight + 4, size: 9, color: rgb(0, 0, 0) })
            }
        })
    }
    ...
    for (const row of tabledata) {
        // start a new page (repeating the header) as soon as the next row would cross the bottom margin
        if (startY - rowOnPage * rowHeight - rowHeight < bottomMargin) {
            page = pdfDoc.addPage()
            startY = page.getHeight() - 50
            rowOnPage = 0
            drawRow(headerRow, startY)
            rowOnPage++
        }
        drawRow(row, startY - rowOnPage * rowHeight)
        rowOnPage++
    }
...
export async function buildTeacherCombinedLatestPdf({ workdirectory, backupdirectory, mcServer, submissions }) {
    ...
    let indexPDFdata = null
    try {
        indexPDFdata = await createIndexPDF(submissions, serverFolder, mcServer)
    } catch (err) {
        // never let a broken index abort combined.pdf
        log.error('getLatestCombinedPdf @ buildTeacherCombinedLatestPdf: index creation failed', err)
    }
    let indexPDFpath = indexPDFdata ? resolvePathUnderRoot(workdirectory, [serverFolder, 'index.pdf']) : null
    try {
        if (!indexPDFpath) {
            if (indexPDFdata) { log.error('getLatestCombinedPdf @ buildTeacherCombinedLatestPdf: rejected index.pdf path') }
        } else {
