...
import { resolveWindows11IsoUrl, MS_DOWNLOAD_PAGE, MS_PROFILE } from '../../../../shared/qemuWindowsIsoUrl.js';
...
const MIN_COMPLETE_BYTES = {
    [DEFAULTS.isoName]: 4 * 1024 * 1024 * 1024,
...
/** User-typed VM name → safe filename inside the QEMU dir (no paths, no overlay names). */
function sanitizeNewDiskName(rawName) {
    const trimmed = String(rawName || '').trim();
    const withExt = /\.qcow2$/i.test(trimmed) ? trimmed : `${trimmed}.qcow2`;
    if (!trimmed || withExt !== path.basename(withExt) || isQemuOverlayDiskName(withExt)) {
        throw new Error('invalid disk name');
    }
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]*\.qcow2$/.test(withExt)) {
        throw new Error('invalid disk name');
    }
    return withExt;
}

/** Never boot or overwrite an existing disk: next free win11-N.qcow2 as install default. */
function getFreeDiskName(qemuDir) {
    let name = DEFAULTS.diskName;
    for (let n = 2; fs.existsSync(path.join(qemuDir, name)); n++) {
        name = `win11-${n}.qcow2`;
    }
    return name;
}

/** Suggested name for the install dialog (without .qcow2). */
async function suggestDiskName({ workdirectory }) {
    const qemuDir = getQemuDir(workdirectory);
    await ensureDir(qemuDir);
    return getFreeDiskName(qemuDir).replace(/\.qcow2$/i, '');
}

async function ensureDisk(qemuDir, diskName = null) {
    const name = diskName ? sanitizeNewDiskName(diskName) : getFreeDiskName(qemuDir);
    const diskPath = path.join(qemuDir, name);
    if (fs.existsSync(diskPath)) {
        throw new Error(`disk already exists: ${name}`);
    }
...
/** Chrome UA matching Electron's Chromium — UA/TLS mismatch is a Sentinel bot signal. */
...
/**
 * Resolve the Win11 ISO link by driving the REAL microsoft.com download page UI inside a
 * hidden BrowserWindow (select edition → download → select language → read 64-bit link).
 * Replaying the API URLs outside the page session gets Sentinel-rejected even from IPs where
 * the website flow works — the page binds the download to its own session/challenge, so the
 * only reliable automated path is the page's own flow. language must match shared default.
 */
async function resolveIsoUrlViaHiddenBrowser({ language = 'German', onLog = null } = {}) {
    const { BrowserWindow } = await import('electron');
    const log_ = (m) => { try { onLog?.(m); } catch (e) {} };
    const win = new BrowserWindow({
        show: false,
        webPreferences: { sandbox: true, contextIsolation: true, nodeIntegration: false, partition: 'nxe-ms-iso-download' },
    });
    try {
        win.webContents.setUserAgent(buildChromeUserAgent());
        log_(`loading ${MS_DOWNLOAD_PAGE} in hidden window`);
        await win.loadURL(MS_DOWNLOAD_PAGE);
        const script = `(async () => {
            ...
            const pageError = () => {
                const el = document.querySelector('#errorModalMessage, .alert-danger, [id*="error" i][class*="message" i]');
                const txt = el && el.textContent ? el.textContent.trim() : '';
                return txt ? txt.slice(0, 300) : null;
            };
            try {
                const editionSel = await waitFor(() => {
                    const sels = Array.from(document.querySelectorAll('select'));
                    return sels.find((s) => Array.from(s.options).some((o) => /^\\d+$/.test(o.value) && /Windows/i.test(o.textContent)));
                }, 30000);
                if (!editionSel) return { ok: false, error: 'edition dropdown not found' };
                const editionOpt = Array.from(editionSel.options).find((o) => /^\\d+$/.test(o.value) && /Windows/i.test(o.textContent));
                editionSel.value = editionOpt.value;
                editionSel.dispatchEvent(new Event('change', { bubbles: true }));
                const editionBtn = document.querySelector('#submit-product-edition')
                    || (editionSel.closest('div,section,form') || document).querySelector('button');
                if (!editionBtn) return { ok: false, error: 'edition submit button not found' };
                editionBtn.click();

                const langSel = await waitFor(() => {
                    const byId = document.querySelector('#product-languages');
                    if (byId && byId.options.length > 1) return byId;
                    const sels = Array.from(document.querySelectorAll('select'));
                    return sels.find((s) => s !== editionSel && s.options.length > 1
                        && Array.from(s.options).some((o) => /German|Deutsch|English/i.test(o.textContent))) || null;
                }, 45000);
                if (!langSel) return { ok: false, error: 'language dropdown not found; page says: ' + (pageError() || '-') };
                const langWanted = ${JSON.stringify(language)};
                const langOpt = Array.from(langSel.options).find((o) =>
                    (o.textContent + ' ' + o.value).match(new RegExp(langWanted === 'German' ? 'German|Deutsch' : langWanted, 'i')));
                if (!langOpt) return { ok: false, error: 'language option not found: ' + Array.from(langSel.options).map((o) => o.textContent.trim()).join(', ') };
                langSel.value = langOpt.value;
                langSel.dispatchEvent(new Event('change', { bubbles: true }));
                const langBtn = document.querySelector('#submit-sku')
                    || (langSel.closest('div,section,form') || document).querySelector('button');
                if (!langBtn) return { ok: false, error: 'language submit button not found' };
                langBtn.click();

                const link = await waitFor(() => {
                    const anchors = Array.from(document.querySelectorAll('a[href*=".iso"]'));
                    const a = anchors.find((x) => /x64|64-bit|64 bit/i.test(x.textContent + ' ' + x.href)) || anchors[0];
                    return a ? a.href : null;
                }, 45000);
                if (!link) return { ok: false, error: '64-bit link not found; page says: ' + (pageError() || '-') };
                return { ok: true, uri: link };
            } catch (e) { return { ok: false, error: String((e && e.message) || e) }; }
        })()`;
        const res = await Promise.race([
            win.webContents.executeJavaScript(script, true),
            new Promise((_, rej) => setTimeout(() => rej(new Error('hidden-browser resolve timeout')), 150000)),
        ]);
        if (!res?.ok || !res.uri) throw new Error(res?.error || 'hidden-browser resolve failed');
        log_('got iso link via hidden window (page flow)');
...
async function installDefaultVm({ workdirectory, isoPath: manualIsoPath = null, diskName: requestedDiskName = null, onProgress = null }) {
    const qemuDir = getQemuDir(workdirectory);
    await ensureDir(qemuDir);

    log.info(`qemuService @ installDefaultVm: requested (workdirectory=${workdirectory}, manualIso=${manualIsoPath || '-'})`);
    let isoPath;
    if (manualIsoPath) {
        // manually selected install ISO: use in place, no copy into QEMU dir
        isoPath = path.resolve(String(manualIsoPath));
        if (!isoPath.toLowerCase().endsWith('.iso')) throw new Error('invalid iso file');
        await fs.promises.access(isoPath, fs.constants.R_OK);
    } else {
        // no static Microsoft links → resolve a fresh (~24h valid) link via microsoft.com API
        isoPath = path.join(qemuDir, DEFAULTS.isoName);
        if (!isCompleteDownload(isoPath, DEFAULTS.isoName)) {
            try { onProgress?.({ phase: 'resolving-iso-url', file: DEFAULTS.isoName, percent: 0 }); } catch (e) {}
            let isoUrl;
            try {
                isoUrl = await resolveIsoUrlViaHiddenBrowser({
                    onLog: (m) => log.info(`qemuService @ resolveIsoUrlViaHiddenBrowser: ${m}`),
                });
            } catch (e) {
                log.warn(`qemuService @ installDefaultVm: hidden-browser resolve failed (${e?.message || e}), falling back to net.fetch`);
                // net.fetch fallback: Chromium TLS fingerprint, Chrome UA
                const { net } = await import('electron');
                isoUrl = await resolveWindows11IsoUrl({
                    fetchImpl: net.fetch.bind(net),
                    userAgent: buildChromeUserAgent(),
                    onLog: (m) => log.info(`qemuService @ resolveWindows11IsoUrl: ${m}`),
                });
            }
            await downloadFile(isoUrl, isoPath, onProgress);
        } else {
            try { onProgress?.({ phase: 'skip', file: DEFAULTS.isoName, percent: 100 }); } catch (e) {}
        }
    }
    ...
    const diskPath = await ensureDisk(qemuDir, requestedDiskName);
    const diskName = path.basename(diskPath);
    try { onProgress?.({ phase: 'starting-qemu', file: diskName, percent: 0 }); } catch (e) {}
    ...
    return { ok: true, qemuDir, diskName, vncDisplay: DEFAULTS.vncDisplay };
}
