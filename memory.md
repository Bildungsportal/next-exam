...
PATH^student^appleHelpers^scripts/apple/{assessment.swift→assessment-helper,wifi.swift→wifi-helper}; build.sh builds both; build-apple.mjs builds+signs both w/ shared scripts/entitlements.mac.plist (NXE_APPLE_ADHOC=1 local); npm run build:apple(:local); beforepack builds, afterpack signs both in bundle Resources/apple/; extraResources apple/; assessmentSession.js=assessment start/stop; getwlaninfo.js getWlanInfoMacOSHelper spawns wifi-helper (CoreWLAN+CoreLocation) one-shot JSON {ssid,bssid,rssi,message}
RULE^student^macWifiInfo^SSID/BSSID on macOS14+ need com.apple.developer.networking.wifi-info entitlement + Location auth; shell tools (airport/networksetup/ipconfig/system_profiler) return redacted→only native wifi-helper works; networking.* entitlements MAY need embedded provisioningProfile (multicast crashed w/o one) - AAC did not; quasar.config mac block has none yet
