...
RULE^win32^kioskAutoLogoff^triggerWindowsKioskLogoff shutdown /l /f; PRIMARY wipe: electron-main will-quit fs.rmSync(config.workdirectory) when runningInCage; BACKUP wipe: NextExam-KioskWipeUserHome scheduled task -AtStartup (Security 4634 unreliable - depends on audit policy)
...
PATH^student^netScan^networkActiveProcesses.js scans non-loopback TCP established + TCP LISTEN; excludes next-exam subtree + LT pid + LT cmdline markers + sys-critical allowlist (win: spoolsv jhi_service + core OS); findNonLanguageToolOn8088=listen on 8088 not LT; win32 collectWin32 1 PS JSON
