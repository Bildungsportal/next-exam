...
TECH^vite^pdfjsLegacyAlias^quasar alias pdfjs legacy paths via resolvePdfJsFile fallbacks; shared/pdfparser imports legacy/build; prebuild assertPdfJsDist checks pdf.mjs+worker exist (incomplete npm install = only .map)^student+teacher quasar.config+prebuild.js
...
PATH^win32^kioskInstall^install-windows-kiosk.ps1+windowsKioskSetup.js; needsCageKioskSetup=!.kiosk-provision-complete marker (not exe+user alone); marker written only on ps1 DONE; UAC+SYSTEM task MDM; C:\NextExam\next-exam.exe+user next-exam-kiosk
