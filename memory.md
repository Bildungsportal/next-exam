...
TECH^submissionSign^pades^auto always; bip=userprivateaccesskey in main setBipSiteInfo after site_info; local=sha256(pin|token|timeMs); cert OU NXE-SALT+NXE-MODE(bip|local)+NXE-BIP-UID; teacher verify integrity|verifySubmissionPdfViaBip BiP login^shared/submissionPdfSign.js+student communicationhandler+teacher ipchandler
