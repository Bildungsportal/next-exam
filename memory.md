...
RULE^win32^kioskAllowedAppsDrift^read once at startup in getWindowsKioskDetectionLogLines→initAllowedKioskAppsAtSessionStart; syncAllowedKioskAppsClientinfo only attaches cache to clientinfo (NOT per requestUpdate tick); LayoutModification.xml+HKLM AllowedApps reg /s; notInLauncherJson+notInLauncherJsonAumids
