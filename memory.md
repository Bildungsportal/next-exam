...
IPC^win32^kioskExitCodes^ps1 exits 10=EDITION_UNSUPPORTED→main returns {ok:false,code:'EDITION_UNSUPPORTED'}→student.vue shows friendly editionHint; UAC via -EncodedCommand UTF-16LE base64 + exitFile workaround (Start-Process ExitCode unreadable across elevation boundary)
