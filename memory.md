...
PATH^win32^kioskInstall^src-electron/resources/win32/install-windows-kiosk.ps1 (extraResources→win32/) + src-electron/main/scripts/win/windowsKioskSetup.js; UAC Start-Process -Verb RunAs; edition gate Pro|Edu|Ent; copy full Electron bundle dirname(process.execPath)→C:\NextExam (portable unpack %TEMP%\next-exam-student, launch Next-Exam-Student.exe); not single-file rename; .kiosk-launch-exe.txt marker; user next-exam-kiosk; Set-LocalUserPasswordlessLogon(net passwordreq:no+ADSI PasswordExpired=0+UF_PASSWD_NOTREQD)+LSA LimitBlankPasswordUse=1; userenv!CreateProfile+offline hive hardening; State=128; RemovableStorageDevices\<SID>; MDM_AssignedAccess allow list must include Next-Exam exe + bundled java.exe/javaw.exe + disable-shortcuts.exe (child spawn blocked otherwise)
RULE^win32^kioskStartUi^StartPins desktopAppLink MUST be absolute path (Win11 does not expand %USERPROFILE%/%ALLUSERSPROFILE%); .lnk in C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Next-Exam; JSON backslashes single-escape (\\ in source); ConfigureStartPins via HKLM machine policy (no NTUSER reload); applyOnce=false; kiosk-launcher-apps.json drives in-app launcher bar (student.vue) and is the primary path-to-apps when StartPins fails
...
RULE^pdfparser^isClozeField^all clozeFields.push set isClozeField:true; filterDegenerateInteractiveFields exempts isClozeField (like checkbox/deselect) so narrow markers ("__10__" 2-underscore math worksheets) survive 22px minW gate; underscore push allows ≥6px^shared/pdfparser/shared/filters.js+shared/pdfparser/v5/detectors.js
