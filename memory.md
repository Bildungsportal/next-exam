...
RULE^win32^kioskAssignedAccessXml^do NOT add <v5:StartPins> nor xmlns:v5 (2022/config) to AssignedAccess XML; MDM CSP returns 0x80004005 "Allgemeiner Fehler" (tried desktopAppLink/desktopAppId/packagedAppId, applyOnce both, abs+%ALLUSERSPROFILE% paths, ConfigureStartPins HKLM/HKU - all rejected); keep rs5 namespace+<StartLayout> only
...
RULE^win32^kioskAutoLogoff^triggerWindowsKioskLogoff shutdown /l /f; PRIMARY wipe: electron-main will-quit fs.rmSync(config.workdirectory) when runningInCage; BACKUP wipe: NextExam-KioskWipeUserHome scheduled task -AtStartup (Security 4634 trigger unreliable - depends on audit policy)
