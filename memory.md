...
RULE^win32^kioskStartUi^launchers via strict JSON {"apps":[{name,path}]} at C:\NextExam\kiosk-launcher-apps.json; rendered in student.vue + ExamHeader.vue via loadWinKioskLauncherApps; $skipLauncherUi (java/javaw/disable-shortcuts/netsh/powershell/reg.exe) hides internals; netsh+powershell+reg.exe are AllowedApps for next-exam spawn only, never student-facing buttons
...
RULE^win32^kioskDetect^windowsKioskSetup.js: reg.exe+whoami spawns fail under AA if not AllowedApps; profileState128 fallback USERPROFILE ends with next-exam-kiosk; sid fallback .kiosk-account-sid for ProfileList reg read; aaProof=RestrictRun AssignedAccess_*|MDM CSP|Winlogon shell (multi-app AA often no shell match)
