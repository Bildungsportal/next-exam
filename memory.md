...
RULE^win32^kioskStartUi^AllAppsList=run permission only; install-windows-kiosk.ps1 pins every AllowedApps entry via StartLayout DesktopApplicationLink+Taskbar ShowTaskbar=true; kiosk-allowed-apps.txt paths appear after re-provision not from whitelist alone
